...
class Ship:
    ...
    def selectShipAndMove(self):
        ...
        while self.active == True:
            ...
            updateGameScreen(GAMESCREEN, GAMESTATE,player1Wins)
...
class ComputerFactory:
    @staticmethod
    def create_computer(difficulty):
        if difficulty == "easy":
            return EasyComputer()
...
class ExplosionAnimationStrategy(AnimationStrategy):
    def animate(self, token):
        token.explosionIndex += 1
        if token.explosionIndex < len(token.explosionList):
            return token.explosionList[token.explosionIndex]
        else:
            return FireAnimationStrategy().animate(token)

class FireAnimationStrategy(AnimationStrategy):
    def animate(self, token):
        if pygame.time.get_ticks() - token.timer >= 100:
            token.timer = pygame.time.get_ticks()
            token.imageIndex += 1
        if token.imageIndex < len(token.imageList):
            return token.imageList[token.imageIndex]
        else:
            token.imageIndex = 0
            return token.imageList[token.imageIndex]
...
class Tokens:
    def __init__(self, image, pos, action, imageList=None, explosionList=None, soundFile=None):
        ...
        self.animation_strategy = ExplosionAnimationStrategy() if explosionList else None

    def draw(self, window):
        if self.animation_strategy:
            self.image = self.animation_strategy.animate(self)
...
def endScreen(window,player):
    ...
    text_rect = text_surface.get_rect(midtop=(window.get_width() // 2, window.get_height() // 4))
    window.blit(text_surface, text_rect)

    # Виведення фонового зображення (якщо потрібно)
    # window.blit(ENDSCREENIMAGE, (0, 0))
...
BUTTONS = [
    Button(BUTTONIMAGE, (150, 50), (700, 600), 'Randomize'),
    Button(BUTTONIMAGE, (150, 50), (900, 600), 'Reset'),
    Button(BUTTONIMAGE, (150, 50), (1100, 600), 'Deploy'),
...
easy_computer = computer_factory.create_computer("easy")
hard_computer = computer_factory.create_computer("hard")
